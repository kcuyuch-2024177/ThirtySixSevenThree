...
// Valida productoId y cantidad. Devuelve { error, status } si hay fallo,
// o { productoId, cantidad } si todo está bien.
function validarProductoYCantidad(productoId, cantidad) {
  if (!productoId || !mongoose.Types.ObjectId.isValid(productoId)) {
    return {
      error: 'El productoId es obligatorio y debe ser un id válido',
      status: 400,
    };
  }
